...
M_a = [0 0 0; .524*(10^-6) p_surv*(11/12) 0; 0 p_surv*1/12 p_surv*1]; %aging matrix
...
for k = 1:nBeds
    hold on
    plot(time,mature(:,k),'LineWidth',1);
end
ylim padded;
set(gca,'FontSize',24)
xlabel('Time');
...
M_r = [0 0 fec*b_0; .524*(10^-6) p_surv*(11/12) 0;  0 p_surv*1/12 p_surv*1]; %aging during reproduction
M_a = [0 0 0; .524*(10^-6) p_surv*(11/12) 0; 0 p_surv*1/12 p_surv*1]; %aging matrix
...
for k = 1:steps
    if mod(k,4) ==0
        state_copy = M_r*state;
    else
        state_copy = M_a*state;
    end
    state = state_copy;
    mature(k+1,:) = state(3,:);
end
