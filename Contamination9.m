...
omega_age = 0.5;
omega_repro = 0.5;
cont_mass = 1000; %total cont mass
...
for step_counter = 1:steps
    ...
    for bed_counter = 1:nBeds
        ...
        curr_mass = curr_mass - cont_removed;
...
for k = 3:nBeds-2
    hold on
    plot(0:steps,mature(:,k),'LineWidth',2,'Marker',MarkerShapes(mod(k+2,5)+1),'MarkerSize',12);
end
ylim padded;
legend('3','4','5','6','7')
title("Contaminant")
...
for k = 3:nBeds-2
    hold on
    plot(0:steps,abs(mature_Dem9(:,k)-mature(:,k))./mature_Dem9(:,k),'LineWidth',2,'Marker',MarkerShapes(mod(k+2,5)+1),'MarkerSize',12);
end
ylim padded;
legend('3','4','5','6','7')
title("Contaminant")
